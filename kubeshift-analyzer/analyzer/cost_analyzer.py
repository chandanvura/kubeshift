...
class CostAnalyzer:
    ...
    def analyze_costs(self, metrics: List[ResourceMetrics]) -> List[CostAnalysis]:
        analyses = []
        HOURS_PER_MONTH = 730
        ...
        for metric in metrics:
            ...
            monthly_cost = (cpu_req_cores * self.config.CPU_RATE_PER_CORE_HOUR + mem_req_gb * self.config.MEMORY_RATE_PER_GB_HOUR) * HOURS_PER_MONTH
            
            cpu_usage_cores = metric.cpu_usage_millis / 1000.0
            mem_usage_gb = metric.memory_usage_bytes / (1024 ** 3)
            
            optimized_cost = ((cpu_usage_cores * 1.2) * self.config.CPU_RATE_PER_CORE_HOUR + (mem_usage_gb * 1.2) * self.config.MEMORY_RATE_PER_GB_HOUR) * HOURS_PER_MONTH
            ...
            avg_util = (metric.cpu_utilization_percent + metric.memory_utilization_percent) / 2
            
            if avg_util < 30:
                is_overprovisioned = True
                recommendations.append("Severely overprovisioned. Consider reducing limits by at least 50%.")
            elif avg_util < 50:
                is_overprovisioned = True
                recommendations.append("Moderately overprovisioned. Consider right-sizing to match usage + 20% buffer.")
    ...
    def get_namespace_summary(self, analyses: List[CostAnalysis]) -> Dict[str, dict]:
        summary = {}
        ...
        for ns in summary:
            ...
            summary[ns]["total_savings"] = round(summary[ns]["total_savings"], 2)
