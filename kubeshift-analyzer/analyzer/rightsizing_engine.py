...
class RightsizingEngine:
    ...
    def generate_recommendations(self, metrics: List[ResourceMetrics]) -> List[RightsizingRecommendation]:
        recommendations = []
        HOURS_PER_MONTH = 730
        ...
        for metric in metrics:
            ...
            if metric.cpu_utilization_percent < 50:
                ...
                if recommended_cpu < metric.cpu_request_millis:
                    savings = ((metric.cpu_request_millis - recommended_cpu) / 1000.0) * self.config.CPU_RATE_PER_CORE_HOUR * HOURS_PER_MONTH
            ...
            if metric.memory_utilization_percent < 50:
                recommended_mem = max(64 * 1024 * 1024, metric.memory_usage_bytes * 1.3)
                if recommended_mem < metric.memory_request_bytes:
                    savings = ((metric.memory_request_bytes - recommended_mem) / (1024 ** 3)) * self.config.MEMORY_RATE_PER_GB_HOUR * HOURS_PER_MONTH
                    recommendations.append(RightsizingRecommendation(
                        namespace=metric.namespace,
                        deployment_name=metric.deployment_name,
                        container_name=metric.container_name,
