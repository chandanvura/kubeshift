...
class AnomalyDetector:
    ...
    def _get_key(self, metric: ResourceMetrics) -> str:
        return f"{metric.namespace}/{metric.deployment_name}/{metric.container_name}"
    ...
    def detect_anomalies(self, metrics: List[ResourceMetrics]) -> List[AnomalyAlert]:
        alerts = []
        ...
        for metric in metrics:
            ...
            if abs_z > self.config.ANOMALY_THRESHOLD:
                ...
                desc = "Cost spike detected" if z_score > 0 else "Sudden utilization drop detected"
