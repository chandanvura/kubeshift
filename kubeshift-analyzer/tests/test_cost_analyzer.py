...
def test_overprovisioned_detection(test_settings, sample_metrics):
    ...
    # The 4th item in mock data is staging/api-gateway (10% cpu, 15% mem) -> should be overprovisioned
    staging_gateway = next((a for a in analyses if a.deployment_name == "api-gateway" and a.namespace == "staging"), None)
    assert staging_gateway is not None
    assert staging_gateway.is_overprovisioned == True
...
def test_well_provisioned_detection(test_settings, sample_metrics):
    ...
    # user-service has 80% cpu, 85% mem utilization -> should not be overprovisioned
    user_service = next((a for a in analyses if a.deployment_name == "user-service"), None)
    assert user_service is not None
    assert user_service.is_overprovisioned == False
